...
def main():
    # 取得上傳目標與檔案路徑
    upload_type = getattr(settings, "UPLOAD_SUMMARY_REPORT_TARGET", ["all"])  # r2, gdrive, supabase
    targets = upload_type if isinstance(upload_type, list) else [upload_type]

    # 設定檔案路徑
    csv_path = getattr(settings, "REPORT_CSV_DIR", "datas/reports_data/csv/summary_report.csv")
    json_path = getattr(settings, "REPORT_JSON_DIR", "datas/reports_data/json/summary_report.json")
